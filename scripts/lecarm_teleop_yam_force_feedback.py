...
def main():
    ...
    try:
        ...
        while True:
            ...
            yam_joint_eff = yam_robot.get_observations()["joint_eff"][:6]
            yam_gravity_torque = yam_robot.get_compensated_torque()[:6]
            kd = np.array([5, 5, 5, 1.5, 1.5, 1.5]) * 0.20
            
            # Element-wise scaling of torque difference
            torque_scale = np.array([0.2, 0.15, 0.15, 0.8, 0.8, 0.8])
            feedback_torque = (yam_joint_eff - yam_gravity_torque) * torque_scale
            lecarm_robot.send_gravity_compensation_and_force_feedback(feedback_torque, kd=kd)
            print(f"lecarm_feedback_torque: {yam_joint_eff - yam_gravity_torque}")
